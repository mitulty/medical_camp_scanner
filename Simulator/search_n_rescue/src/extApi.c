...
simxVoid _softLock() { /* a bit delicate, since we can't use mutexes yet... very simple non fail-safe lock! */
	while (1) {
		if (_softLock_ == 0) {
			_softLock_ = 1;
			return;
...
simxVoid _softUnlock() { /* a bit delicate, since we can't use mutexes yet... very simple non fail-safe unlock! */
	_softLock_ = 0;
}
...
EXTAPI_DLLEXPORT simxInt simxStart(const simxChar *connectionAddress,
		simxInt connectionPort, simxUChar waitUntilConnected,
		simxUChar doNotReconnectOnceDisconnected, simxInt timeOutInMs,
	...
	if (clientID != -1) {
		for (i = 0; i < MAX_EXT_API_CONNECTIONS; i++) {
			if (_nextConnectionID[i] != 0) {
				if (connectionPort < 0) { /* using shared memory */
					if (connectionPort == _connectionPort[i]) { /* that 'shared memory number' was already used */
						clientID = -1;
						break;
					}
				} else { /* using sockets */
					if ((connectionPort == _connectionPort[i])
							&& (extApi_areStringsSame(_connectionIP[i],
									connectionAddress) != 0)) { /* that IP/port was already used */
						clientID = -1;
						break;
					}
				}
	...
	if (connectionPort < 0) { /* using shared memory */
		...
		_connectionIP[clientID] = (simxChar*) extApi_allocateBuffer(
				extApi_getStringLength(connectionAddress) + 1);
	...
	_splitCommandsReceived[clientID] = extApi_allocateBuffer(
			SIMX_INIT_BUFF_SIZE);
	...
	if (timeOutInMs < 0) { /* a positive time out is for the first connection. A negative, for all blocking commands */
	...
	_tempDoNotReconnectOnceDisconnected[clientID] =
	...
	while ((extApi_getTimeDiffInMs(startTime) < timeOutInMs)
			&& (_connectionID[clientID] == -1))
		extApi_switchThread();
	if (_connectionID[clientID] == -1) { /* we failed connecting */
		simxFinish(clientID);
		return (-1);
	}
	return (clientID);
...
EXTAPI_DLLEXPORT simxVoid simxFinish(simxInt clientID) {
	...
	if (clientID >= 0) { /* shut down a specific client */
		if (_communicationThreadRunning[clientID]) {

			if (_connectionID[clientID] != -1) { /* we are still connected */
				/* send the kill connection command */
				_exec_int(clientID, simx_cmd_kill_connection,
						simx_opmode_oneshot, 0, 0, &returnValue);
				/* wait 0.5 seconds */
				extApi_sleepMs(500);
			}
	...
	} else { /* shut down all opened clients */
		for (i = 0; i < MAX_EXT_API_CONNECTIONS; i++) {
			if (_communicationThreadRunning[i]) {
				if (_connectionID[i] != -1) { /* we are still connected */
					/* send the kill connection command */
					_exec_int(i, simx_cmd_kill_connection, simx_opmode_oneshot,
							0, 0, &returnValue);
				}
			}
		}
...
simxVoid _waitUntilMessageArrived(simxInt clientID, simxInt *error) {
	...
	if (_waitBeforeSendingAgainWhenMessageIDArrived[clientID] != -1) { /* wait until we received a reply, or a timeout (if we wanna wait for the reply) */
		startTime = extApi_getTimeInMs();
		while (1) {
			extApi_lockResources(clientID);
			lastReceivedMessageIDCopy = _lastReceivedMessageID[clientID];
			extApi_unlockResources(clientID);
			if ((extApi_getTimeDiffInMs(startTime)
					>= _replyWaitTimeoutInMs[clientID])
					|| (lastReceivedMessageIDCopy
							>= _waitBeforeSendingAgainWhenMessageIDArrived[clientID]))
				break;
			extApi_switchThread();
		}
		if (lastReceivedMessageIDCopy
				< _waitBeforeSendingAgainWhenMessageIDArrived[clientID])
			error[0] |= simx_return_timeout_flag;
	}
}

simxUChar* _setLastFetchedCmd(simxInt clientID, simxUChar *cmdPtr,
		simxInt *error) {
	simxInt blockSize, incr, i, status;
	simxUChar *newCommand;
	if (cmdPtr != 0) {
		blockSize = extApi_endianConversionInt(
				((simxInt*) (cmdPtr + simx_cmdheaderoffset_mem_size))[0]);
		if (blockSize > _commandReceived_bufferSize[clientID]) { /* we need more memory here for the fetched command buffer */
			incr = blockSize - _commandReceived_bufferSize[clientID];
			if (incr < SIMX_MIN_BUFF_INCR)
				incr = SIMX_MIN_BUFF_INCR;
			newCommand = (simxUChar*) extApi_allocateBuffer(
...
simxUChar* _exec_null(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt *error) {
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			else { /* Command not there. Add it */
				_splitCommandsToSend[clientID] = _appendCommand_(
						cmdRaw + opMode, options, delayOrSplit,
						_splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
		} else {
			cmdPtr = _getCommandPointer_(cmdRaw,
					_messageToSend[clientID] + SIMX_HEADER_SIZE,
					_messageToSend_dataSize[clientID] - SIMX_HEADER_SIZE);
			if ((cmdPtr == 0) || (options & 1)) { /* Command not there (or cmd cannot be overwritten). Add it */
				_messageToSend[clientID] = _appendCommand_(cmdRaw + opMode,
						options, delayOrSplit, _messageToSend[clientID],
...
simxUChar* _exec_null_buffer(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxUChar *buffer, simxInt bufferSize,
		simxInt *error) {
	simxUShort delayOrSplit;
	simxUChar *cmdPtr = 0;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr == 0)
				_splitCommandsToSend[clientID] = _appendCommand_null_buff(
						cmdRaw + opMode, options, buffer, bufferSize,
			...
			_messageToSend[clientID] = _appendCommand_null_buff(cmdRaw + opMode,
					options, buffer, bufferSize, delayOrSplit,
...
simxUChar* _exec_int(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue, simxInt *error) {
	simxUShort delayOrSplit;
	simxUChar *cmdPtr = 0;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			if (delayOrSplit < _MIN_SPLIT_AMOUNT_IN_BYTES)
				delayOrSplit = _MIN_SPLIT_AMOUNT_IN_BYTES;
			cmdPtr = _getCommandPointer_i(cmdRaw, intValue,
					_splitCommandsToSend[clientID],
					_splitCommandsToSend_dataSize[clientID]);
			if (cmdPtr != 0)
				error[0] |= simx_return_split_progress_flag; /* Command already there */
			else { /* Command not there. Add it */
				_splitCommandsToSend[clientID] = _appendCommand_i(
						cmdRaw + opMode, options, intValue, delayOrSplit,
						_splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
			...
			cmdPtr = _getCommandPointer_i(cmdRaw, intValue,
					_messageToSend[clientID] + SIMX_HEADER_SIZE,
					_messageToSend_dataSize[clientID] - SIMX_HEADER_SIZE);
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0] =
						extApi_endianConversionInt(cmdRaw + opMode);
			} else { /* Command not there. Add it */
				_messageToSend[clientID] = _appendCommand_i(cmdRaw + opMode,
						options, intValue, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_intint(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue1, simxInt intValue2, simxInt *error) {
	simxUShort delayOrSplit;
	simxUChar *cmdPtr = 0;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			if (delayOrSplit < _MIN_SPLIT_AMOUNT_IN_BYTES)
				delayOrSplit = _MIN_SPLIT_AMOUNT_IN_BYTES;
			cmdPtr = _getCommandPointer_ii(cmdRaw, intValue1, intValue2,
					_splitCommandsToSend[clientID],
					_splitCommandsToSend_dataSize[clientID]);
			if (cmdPtr != 0)
				error[0] |= simx_return_split_progress_flag; /* Command already there */
			else { /* Command not there. Add it */
				_splitCommandsToSend[clientID] = _appendCommand_ii(
						cmdRaw + opMode, options, intValue1, intValue2,
						delayOrSplit, _splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
			...
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				...
				_messageToSend[clientID] = _appendCommand_ii(cmdRaw + opMode,
						options, intValue1, intValue2, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_string(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, const simxUChar *stringValue, simxInt *error) {
	simxUShort delayOrSplit;
	simxUChar *cmdPtr = 0;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			if (delayOrSplit < _MIN_SPLIT_AMOUNT_IN_BYTES)
				delayOrSplit = _MIN_SPLIT_AMOUNT_IN_BYTES;
			cmdPtr = _getCommandPointer_s(cmdRaw, stringValue,
					_splitCommandsToSend[clientID],
					_splitCommandsToSend_dataSize[clientID]);
			if (cmdPtr != 0) { /* Command already there */
				error[0] |= simx_return_split_progress_flag;
			} else { /* Command not there. Add it */
				_splitCommandsToSend[clientID] = _appendCommand_s(
						cmdRaw + opMode, options, stringValue, delayOrSplit,
						_splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
			...
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				...
				_messageToSend[clientID] = _appendCommand_s(cmdRaw + opMode,
						options, stringValue, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_int_int(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue, simxInt intValue2, simxInt *error) {
	simxUChar *cmdPtr;
	simxUShort delayOrSplit;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				...
				_splitCommandsToSend[clientID] = _appendCommand_i_i(
						cmdRaw + opMode, options, intValue, intValue2,
						delayOrSplit, _splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
			...
			cmdPtr = _getCommandPointer_i(cmdRaw, intValue,
					_messageToSend[clientID] + SIMX_HEADER_SIZE,
					_messageToSend_dataSize[clientID] - SIMX_HEADER_SIZE);
			if (cmdPtr != 0) { /* Command already there. Update it */
				((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0] =
						extApi_endianConversionInt(cmdRaw + opMode);
				((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE + 4))[0] =
						extApi_endianConversionInt(intValue2);
			} else { /* Command not there. Add it */
				_messageToSend[clientID] = _appendCommand_i_i(cmdRaw + opMode,
						options, intValue, intValue2, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_intint_int(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue1, simxInt intValue2,
		simxInt intValue3, simxInt *error) {
	simxUChar *cmdPtr;
	simxUShort delayOrSplit;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr != 0) { /* Command already there */
				...
				_splitCommandsToSend[clientID] = _appendCommand_ii_i(
						cmdRaw + opMode, options, intValue1, intValue2,
						intValue3, delayOrSplit, _splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
		} else {
			...
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0] =
						extApi_endianConversionInt(cmdRaw + opMode);
				((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE + 8))[0] =
						extApi_endianConversionInt(intValue3);
			} else { /* Command not there. Add it */
				_messageToSend[clientID] = _appendCommand_ii_i(cmdRaw + opMode,
						options, intValue1, intValue2, intValue3, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_intint_buffer(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue1, simxInt intValue2,
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr != 0) { /* Command already there */
				...
				_splitCommandsToSend[clientID] = _appendCommand_ii_buff(
						cmdRaw + opMode, options, intValue1, intValue2, buffer,
			...
			cmdPtr = _getCommandPointer_ii(cmdRaw, intValue1, intValue2,
					_messageToSend[clientID] + SIMX_HEADER_SIZE,
					_messageToSend_dataSize[clientID] - SIMX_HEADER_SIZE);
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Remove it, we'll add it again just after */
				_removeChunkFromBuffer(_messageToSend[clientID], cmdPtr,
						extApi_endianConversionInt(
								((simxInt*) (cmdPtr
										+ simx_cmdheaderoffset_mem_size))[0]),
			...
			/* Add it: */
			_messageToSend[clientID] = _appendCommand_ii_buff(cmdRaw + opMode,
					options, intValue1, intValue2, buffer, bufferSize,
...
simxUChar* _exec_int_float(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue, simxFloat floatValue,
		simxInt *error) {
	simxUChar *cmdPtr;
	simxUShort delayOrSplit;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr != 0) { /* Command already there */
				...
				_splitCommandsToSend[clientID] = _appendCommand_i_f(
						cmdRaw + opMode, options, intValue, floatValue,
						delayOrSplit, _splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
			}
			...
			if ((cmdPtr != 0) && ((options & 1) == 0)) { /* Command already there, and we can overwrite it. Update it */
				((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0] =
						extApi_endianConversionInt(cmdRaw + opMode);
				((simxFloat*) (cmdPtr + SIMX_SUBHEADER_SIZE + 4))[0] =
						extApi_endianConversionFloat(floatValue);
			} else { /* Command not there. Add it */
				_messageToSend[clientID] = _appendCommand_i_f(cmdRaw + opMode,
						options, intValue, floatValue, delayOrSplit,
						_messageToSend[clientID],
						&_messageToSend_bufferSize[clientID],
						&_messageToSend_dataSize[clientID]);
			}
...
simxUChar* _exec_int_buffer(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, simxInt intValue, simxUChar *buffer,
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr == 0)
				_splitCommandsToSend[clientID] = _appendCommand_i_buff(
						cmdRaw + opMode, options, intValue, buffer, bufferSize,
						delayOrSplit, _splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
		} else {
				...
				_removeChunkFromBuffer(_messageToSend[clientID], cmdPtr,
						extApi_endianConversionInt(
								((simxInt*) (cmdPtr
										+ simx_cmdheaderoffset_mem_size))[0]),
						&_messageToSend_dataSize[clientID]); /* we remove then add the command again (b/c no guarantee the buffer has the same size) */
			_messageToSend[clientID] = _appendCommand_i_buff(cmdRaw + opMode,
					options, intValue, buffer, bufferSize, delayOrSplit,
					_messageToSend[clientID],
					&_messageToSend_bufferSize[clientID],
					&_messageToSend_dataSize[clientID]);
		}
...
simxUChar* _exec_string_buffer(simxInt clientID, simxInt cmdRaw, simxInt opMode,
		simxUChar options, const simxUChar *stringValue, simxUChar *buffer,
		simxInt bufferSize, simxInt *error) {
	simxUChar *cmdPtr;
	simxUShort delayOrSplit;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			...
			if (cmdPtr == 0)
				_splitCommandsToSend[clientID] = _appendCommand_s_buff(
						cmdRaw + opMode, options, stringValue, buffer,
						bufferSize, delayOrSplit,
						_splitCommandsToSend[clientID],
						&_splitCommandsToSend_bufferSize[clientID],
						&_splitCommandsToSend_dataSize[clientID]);
				...
				_removeChunkFromBuffer(_messageToSend[clientID], cmdPtr,
						extApi_endianConversionInt(
								((simxInt*) (cmdPtr
										+ simx_cmdheaderoffset_mem_size))[0]),
						&_messageToSend_dataSize[clientID]); /* we remove then add the command again (b/c no guarantee the buffer has the same size) */
			_messageToSend[clientID] = _appendCommand_s_buff(cmdRaw + opMode,
					options, stringValue, buffer, bufferSize, delayOrSplit,
					_messageToSend[clientID],
					&_messageToSend_bufferSize[clientID],
					&_messageToSend_dataSize[clientID]);
		}
...
simxUChar* _exec_intstringstring_buffer(simxInt clientID, simxInt cmdRaw,
		simxInt opMode, simxUChar options, simxInt intValue,
		const simxUChar *stringValue1, const simxUChar *stringValue2,
		simxUChar *buffer, simxInt bufferSize, simxInt *error) {
	simxUChar *cmdPtr;
	simxUShort delayOrSplit;
	...
	if (opMode != simx_opmode_buffer) {
		...
		if (opMode == simx_opmode_oneshot_split) {
			if (delayOrSplit < _MIN_SPLIT_AMOUNT_IN_BYTES)
				delayOrSplit = _MIN_SPLIT_AMOUNT_IN_BYTES;
			cmdPtr = _getCommandPointer_iss(cmdRaw, intValue, stringValue1,
					stringValue2, _splitCommandsToSend[clientID],
					_splitCommandsToSend_dataSize[clientID]);
			if (cmdPtr != 0) { /* Command already there */
				/* Now make sure we have the same command size, otherwise we have to remove the old cmd and add freshly the new */
				if (extApi_endianConversionInt(
						...
						== SIMX_SUBHEADER_SIZE
								+ extApi_getStringLength(
										(simxChar*) stringValue1)
								+ extApi_getStringLength(
										(simxChar*) stringValue2) + 6
								+ bufferSize)
					error[0] |= simx_return_split_progress_flag; /* ok, we have the same size */
				else { /* we don't have the same size! Remove the old command */
					_removeChunkFromBuffer(_splitCommandsToSend[clientID],
			...
			if (cmdPtr == 0)
				_splitCommandsToSend[clientID] = _appendCommand_iss_buff(
						cmdRaw + opMode, options, intValue, stringValue1,
			...
			cmdPtr = _getCommandPointer_iss(cmdRaw, intValue, stringValue1,
					stringValue2, _messageToSend[clientID] + SIMX_HEADER_SIZE,
					_messageToSend_dataSize[clientID] - SIMX_HEADER_SIZE);
			if ((cmdPtr != 0) && ((options & 1) == 0)) /* Command already there, and we can overwrite it. Remove it and add it again */
				_removeChunkFromBuffer(_messageToSend[clientID], cmdPtr,
						extApi_endianConversionInt(
								((simxInt*) (cmdPtr
										+ simx_cmdheaderoffset_mem_size))[0]),
						&_messageToSend_dataSize[clientID]); /* we remove then add the command again (b/c no guarantee the buffer has the same size) */
			_messageToSend[clientID] = _appendCommand_iss_buff(cmdRaw + opMode,
					options, intValue, stringValue1, stringValue2, buffer,
	...
	extApi_lockResources(clientID);
	cmdPtr = _getCommandPointer_iss(cmdRaw, intValue, stringValue1,
			stringValue2, _messageReceived[clientID] + SIMX_HEADER_SIZE,
			_messageReceived_dataSize[clientID] - SIMX_HEADER_SIZE);
	cmdPtr = _setLastFetchedCmd(clientID, cmdPtr, error);
	_waitBeforeSendingAgainWhenMessageIDArrived[clientID] = -1; /* make sure to enable the communication thread again! */
	extApi_unlockResources(clientID);
	if (opMode == simx_opmode_blocking) /* A cmd reply stays in the inbox always.. except when the mode is simx_opmode_blocking (to avoid polluting the inbox) */
		_removeCommandReply_intstringstring(clientID, cmdRaw, intValue,
				stringValue1, stringValue2);
	return (cmdPtr);
}

simxFloat _readPureDataFloat(simxUChar *commandPointer, simxInt stringCnt,
		simxInt byteOffset) {
	...
	if (commandPointer != 0) {
		...
		while (stringCnt != 0) {
			additionalOffset += extApi_getStringLength(
					(simxChar*) commandPointer + SIMX_SUBHEADER_SIZE
							+ additionalOffset) + 1;
			stringCnt--;
		}
		additionalOffset += byteOffset;
		retVal = extApi_endianConversionFloat(
				extApi_getFloatFromPtr(
						commandPointer + SIMX_SUBHEADER_SIZE
								+ additionalOffset));
	}
	return (retVal);
}

simxUChar _readPureDataChar(simxUChar *commandPointer, simxInt stringCnt,
		simxInt byteOffset) {
	simxUChar retVal = 0;
	simxInt additionalOffset = 0;
	if (commandPointer != 0) {
...
simxInt _readPureDataInt(simxUChar *commandPointer, simxInt stringCnt,
		simxInt byteOffset) {
	simxInt retVal = 0;
	simxInt additionalOffset = 0;
	if (commandPointer != 0) {
...
simxUChar* _getCommandPointer_(simxInt cmdRaw,
	...
	while (offset < commandBufferSize) {
		if ((extApi_endianConversionInt(
				extApi_getIntFromPtr(
						commandBufferStart + offset + simx_cmdheaderoffset_cmd))
				& simx_cmdmask) == cmdRaw) {
		...
		offset += extApi_endianConversionInt(
				extApi_getIntFromPtr(
						commandBufferStart + offset
								+ simx_cmdheaderoffset_mem_size));
	}
	return (retVal);
}

simxUChar* _getCommandPointer_i(simxInt cmdRaw, simxInt intValue,
		const simxUChar *commandBufferStart, simxInt commandBufferSize) {
	simxUChar *retVal = 0;
	simxInt offset = 0;
	while (offset < commandBufferSize) {
		if ((extApi_endianConversionInt(
				extApi_getIntFromPtr(
						commandBufferStart + offset + simx_cmdheaderoffset_cmd))
				& simx_cmdmask) == cmdRaw) {
			if (extApi_getIntFromPtr(
					commandBufferStart + offset + SIMX_SUBHEADER_SIZE)
...
simxUChar* _getCommandPointer_ii(simxInt cmdRaw, simxInt intValue1,
		simxInt intValue2, const simxUChar *commandBufferStart,
...
simxUChar* _getCommandPointer_s(simxInt cmdRaw, const simxUChar *stringValue,
		const simxUChar *commandBufferStart, simxInt commandBufferSize) {
	simxUChar *retVal = 0;
	simxInt offset = 0;
	while (offset < commandBufferSize) {
		if ((extApi_endianConversionInt(
			...
			if (extApi_areStringsSame((simxChar*) stringValue,
					(simxChar*) commandBufferStart + offset
							+ SIMX_SUBHEADER_SIZE) != 0) {
				retVal = (simxUChar*) (commandBufferStart + offset);
				break;
			}
		}
		offset += extApi_endianConversionInt(
				((simxInt*) (commandBufferStart + offset
						+ simx_cmdheaderoffset_mem_size))[0]);
	}
	return (retVal);
}

simxUChar* _getCommandPointer_iss(simxInt cmdRaw, simxInt intValue,
		const simxUChar *stringValue1, const simxUChar *stringValue2,
		const simxUChar *commandBufferStart, simxInt commandBufferSize) {
	simxUChar *retVal = 0;
	simxInt offset = 0;
		...
		if ((extApi_endianConversionInt(
			...
			if (extApi_getIntFromPtr(
				...
				if (extApi_areStringsSame((simxChar*) stringValue1,
						(simxChar*) commandBufferStart + offset
								+ SIMX_SUBHEADER_SIZE + sizeof(simxInt)) != 0) {
					if (extApi_areStringsSame((simxChar*) stringValue2,
							(simxChar*) commandBufferStart + offset
									+ SIMX_SUBHEADER_SIZE + sizeof(simxInt)
									+ extApi_getStringLength(
											(simxChar*) commandBufferStart
													+ offset
													+ SIMX_SUBHEADER_SIZE
													+ sizeof(simxInt)) + 1)
							!= 0) {
...
simxVoid _removeChunkFromBuffer(const simxUChar *bufferStart,
		simxUChar *chunkStart, simxInt chunkSize, simxInt *buffer_dataSize) {
	simxInt i, off, l;
...
simxUChar* _appendCommand_(simxInt cmd, simxUChar options,
	...
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 0);

	((simxInt*) (data + simx_cmdheaderoffset_full_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 0);
	((simxUShort*) (data + simx_cmdheaderoffset_pdata_offset0))[0] =
			extApi_endianConversionUShort(0);
	((simxInt*) (data + simx_cmdheaderoffset_pdata_offset1))[0] =
			extApi_endianConversionInt(0);

	((simxInt*) (data + simx_cmdheaderoffset_cmd))[0] =
			extApi_endianConversionInt(cmd);
	((simxUShort*) (data + simx_cmdheaderoffset_delay_or_split))[0] =
			extApi_endianConversionUShort(delayOrSplit);
	data[simx_cmdheaderoffset_status] = options;
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 0, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_null_buff(simxInt cmd, simxUChar options,
		simxUChar *buffer, simxInt bufferSize, simxUShort delayOrSplit,
		simxUChar *destBuffer, simxInt *destBuffer_bufferSize,
	...
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 0 + bufferSize);
	...
	retVal = _appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 0 + bufferSize,
			destBuffer, destBuffer_bufferSize, destBuffer_dataSize);
	extApi_releaseBuffer(data);
	return (retVal);
}

simxUChar* _appendCommand_i(simxInt cmd, simxUChar options, simxInt intValue,
		simxUShort delayOrSplit, simxUChar *buffer, simxInt *buffer_bufferSize,
		simxInt *buffer_dataSize) {
	simxUChar data[SIMX_SUBHEADER_SIZE + 4];
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4);

	((simxInt*) (data + simx_cmdheaderoffset_full_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4);
	((simxUShort*) (data + simx_cmdheaderoffset_pdata_offset0))[0] =
			extApi_endianConversionUShort(4);
	((simxInt*) (data + simx_cmdheaderoffset_pdata_offset1))[0] =
			extApi_endianConversionInt(0);
	...
	((simxUShort*) (data + simx_cmdheaderoffset_delay_or_split))[0] =
			extApi_endianConversionUShort(delayOrSplit);
	data[simx_cmdheaderoffset_status] = options;
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[0] = extApi_endianConversionInt(
			intValue);
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 4, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_ii(simxInt cmd, simxUChar options, simxInt intValue1,
		simxInt intValue2, simxUShort delayOrSplit, simxUChar *buffer,
		simxInt *buffer_bufferSize, simxInt *buffer_dataSize) {
	simxUChar data[SIMX_SUBHEADER_SIZE + 8];
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 8);

	((simxInt*) (data + simx_cmdheaderoffset_full_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 8);
	((simxUShort*) (data + simx_cmdheaderoffset_pdata_offset0))[0] =
			extApi_endianConversionUShort(8);
	((simxInt*) (data + simx_cmdheaderoffset_pdata_offset1))[0] =
			extApi_endianConversionInt(0);
	...
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[0] = extApi_endianConversionInt(
			intValue1);
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[1] = extApi_endianConversionInt(
			intValue2);
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 8, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_s(simxInt cmd, simxUChar options,
		const simxUChar *stringValue, simxUShort delayOrSplit,
	...
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + strLength + 1);

	((simxInt*) (data + simx_cmdheaderoffset_full_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + strLength + 1);
	((simxUShort*) (data + simx_cmdheaderoffset_pdata_offset0))[0] =
			extApi_endianConversionUShort(strLength + 1);
	((simxInt*) (data + simx_cmdheaderoffset_pdata_offset1))[0] =
			extApi_endianConversionInt(0);
	...
	retVal = _appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + strLength + 1,
			buffer, buffer_bufferSize, buffer_dataSize);
	extApi_releaseBuffer(data);
	return (retVal);
}

simxUChar* _appendCommand_i_i(simxInt cmd, simxUChar options, simxInt intValue,
		simxInt intValue2, simxUShort delayOrSplit, simxUChar *buffer,
		simxInt *buffer_bufferSize, simxInt *buffer_dataSize) {
	simxUChar data[SIMX_SUBHEADER_SIZE + 4 + 4];
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4 + 4);
	...
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[1] = extApi_endianConversionInt(
			intValue2);
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 4 + 4, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_ii_i(simxInt cmd, simxUChar options,
		simxInt intValue1, simxInt intValue2, simxInt intValue3,
		simxUShort delayOrSplit, simxUChar *buffer, simxInt *buffer_bufferSize,
		simxInt *buffer_dataSize) {
	simxUChar data[SIMX_SUBHEADER_SIZE + 4 + 4 + 4];
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4 + 4 + 4);

	((simxInt*) (data + simx_cmdheaderoffset_full_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4 + 4 + 4);
	((simxUShort*) (data + simx_cmdheaderoffset_pdata_offset0))[0] =
			extApi_endianConversionUShort(4 + 4);
	((simxInt*) (data + simx_cmdheaderoffset_pdata_offset1))[0] =
			extApi_endianConversionInt(0);
	...
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[1] = extApi_endianConversionInt(
			intValue2);
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[2] = extApi_endianConversionInt(
			intValue3);
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 4 + 4 + 4, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_i_f(simxInt cmd, simxUChar options, simxInt intValue,
		simxFloat floatValue, simxUShort delayOrSplit, simxUChar *buffer,
		simxInt *buffer_bufferSize, simxInt *buffer_dataSize) {
	simxUChar data[SIMX_SUBHEADER_SIZE + 4 + 4];
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4 + 4);
	...
	((simxInt*) (data + SIMX_SUBHEADER_SIZE))[0] = extApi_endianConversionInt(
			intValue);
	((simxFloat*) (data + SIMX_SUBHEADER_SIZE))[1] =
			extApi_endianConversionFloat(floatValue);
	return (_appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 4 + 4, buffer,
			buffer_bufferSize, buffer_dataSize));
}

simxUChar* _appendCommand_i_buff(simxInt cmd, simxUChar options,
		simxInt intValue, simxUChar *buffer, simxInt bufferSize,
		simxUShort delayOrSplit, simxUChar *destBuffer,
	...
	simxUChar *data = extApi_allocateBuffer(
			SIMX_SUBHEADER_SIZE + 4 + bufferSize);
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(SIMX_SUBHEADER_SIZE + 4 + bufferSize);
	...
	retVal = _appendChunkToBuffer(data, SIMX_SUBHEADER_SIZE + 4 + bufferSize,
			destBuffer, destBuffer_bufferSize, destBuffer_dataSize);
	extApi_releaseBuffer(data);
	return (retVal);
}

simxUChar* _appendCommand_ii_buff(simxInt cmd, simxUChar options,
		simxInt intValue1, simxInt intValue2, simxUChar *buffer,
	...
	retVal = _appendChunkToBuffer(data,
			SIMX_SUBHEADER_SIZE + 4 + 4 + bufferSize, destBuffer,
			destBuffer_bufferSize, destBuffer_dataSize);
	extApi_releaseBuffer(data);
	return (retVal);
}

simxUChar* _appendCommand_s_buff(simxInt cmd, simxUChar options,
		const simxUChar *stringValue, simxUChar *buffer, simxInt bufferSize,
		simxUShort delayOrSplit, simxUChar *destBuffer,
		simxInt *destBuffer_bufferSize, simxInt *destBuffer_dataSize) {
	simxInt i;
	simxUChar *retVal;
	...
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(
					SIMX_SUBHEADER_SIZE
							+ extApi_getStringLength((simxChar*) stringValue)
							+ 1 + bufferSize);
	...
	for (i = 0; i < extApi_getStringLength((simxChar*) stringValue) + 1; i++)
		data[SIMX_SUBHEADER_SIZE + i] = stringValue[i];
	for (i = 0; i < bufferSize; i++)
		data[SIMX_SUBHEADER_SIZE
				+ extApi_getStringLength((simxChar*) stringValue) + 1 + i] =
				buffer[i];
	retVal = _appendChunkToBuffer(data,
			SIMX_SUBHEADER_SIZE
					+ extApi_getStringLength((simxChar*) stringValue) + 1
					+ bufferSize, destBuffer, destBuffer_bufferSize,
			destBuffer_dataSize);
	extApi_releaseBuffer(data);
	return (retVal);
}

simxUChar* _appendCommand_iss_buff(simxInt cmd, simxUChar options,
		simxInt intValue, const simxUChar *stringValue1,
	...
	simxUChar *data = extApi_allocateBuffer(
			SIMX_SUBHEADER_SIZE
					+ extApi_getStringLength((simxChar*) stringValue1)
					+ extApi_getStringLength((simxChar*) stringValue2) + 6
					+ bufferSize);
	((simxInt*) (data + simx_cmdheaderoffset_mem_size))[0] =
			extApi_endianConversionInt(
					SIMX_SUBHEADER_SIZE
							+ extApi_getStringLength((simxChar*) stringValue1)
							+ extApi_getStringLength((simxChar*) stringValue2)
							+ 6 + bufferSize);
	...
	for (i = 0; i < extApi_getStringLength((simxChar*) stringValue2) + 1; i++)
		data[SIMX_SUBHEADER_SIZE + i + 4
				+ extApi_getStringLength((simxChar*) stringValue1) + 1] =
				stringValue2[i];
	for (i = 0; i < bufferSize; i++)
		data[SIMX_SUBHEADER_SIZE
				+ extApi_getStringLength((simxChar*) stringValue1)
				+ extApi_getStringLength((simxChar*) stringValue2) + 6 + i] =
				buffer[i];
	retVal = _appendChunkToBuffer(data,
			SIMX_SUBHEADER_SIZE
					+ extApi_getStringLength((simxChar*) stringValue1)
...
simxUChar* _appendChunkToBuffer(const simxUChar *chunk, simxInt chunkSize,
		simxUChar *buffer, simxInt *buffer_bufferSize, simxInt *buffer_dataSize) {
	simxInt i, incr;
	simxUChar *retBuffer;
	if (buffer_bufferSize[0] - buffer_dataSize[0] < chunkSize) { /* not enough space in the buffer. Allocate more! */
...
simxUChar* _appendCommandToBufferAndTakeIntoAccountPreviouslyReceivedData(
		const simxUChar *cmdPtr, simxUChar *cmdBuffer, simxInt cmdBufferSize,
		const simxUChar *chunk, simxInt chunkSize, simxUChar *buffer,
		simxInt *buffer_bufferSize, simxInt *buffer_dataSize) {
	simxInt cmdRaw, cmdSizeWithoutPureData, previousPureDataSize, totalSize;
	simxUChar *prevCmdPtr;

	cmdRaw = extApi_endianConversionInt(
			((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0]) & simx_cmdmask;
	if (cmdRaw == simx_cmd_read_string_stream) { /* special handling with this command: we merge it with the previous data of that type that was received: */
		prevCmdPtr = _getSameCommandPointer(cmdPtr, cmdBuffer, cmdBufferSize);
		if (prevCmdPtr != 0) { /* we have to merge the data: */
			totalSize = 0;

			cmdSizeWithoutPureData = SIMX_SUBHEADER_SIZE
					+ (simxInt) extApi_endianConversionUShort(
							((simxUShort*) (cmdPtr
			...
			if (previousPureDataSize != 0) {
				...
				buffer = _appendChunkToBuffer(chunk, cmdSizeWithoutPureData,
						buffer, buffer_bufferSize, buffer_dataSize);
				totalSize = totalSize + cmdSizeWithoutPureData;

				/* 2. we append the previous data: */
				buffer = _appendChunkToBuffer(
						prevCmdPtr + cmdSizeWithoutPureData,
						previousPureDataSize, buffer, buffer_bufferSize,
						buffer_dataSize);
				totalSize = totalSize + previousPureDataSize;

				/* 3. we append the current data: */
				buffer = _appendChunkToBuffer(chunk + cmdSizeWithoutPureData,
						chunkSize - cmdSizeWithoutPureData, buffer,
						buffer_bufferSize, buffer_dataSize);
				...
				((simxInt*) (buffer + buffer_dataSize[0] - totalSize
						+ simx_cmdheaderoffset_mem_size))[0] =
						extApi_endianConversionInt(totalSize);
				((simxInt*) (buffer + buffer_dataSize[0] - totalSize
						+ simx_cmdheaderoffset_full_mem_size))[0] =
						extApi_endianConversionInt(totalSize);
		...
		return (_appendChunkToBuffer(chunk, chunkSize, buffer,
				buffer_bufferSize, buffer_dataSize)); /* default behaviour is simply to append */
	} else
		return (_appendChunkToBuffer(chunk, chunkSize, buffer,
				buffer_bufferSize, buffer_dataSize)); /* default behaviour is simply to append */
}

simxUChar* _getSameCommandPointer(const simxUChar *cmdPtr, simxUChar *cmdBuffer,
		simxInt cmdBufferSize) {
	simxInt off, cmd1Raw, cmd2Raw, tmp;

	cmd1Raw = extApi_endianConversionInt(
			((simxInt*) (cmdPtr + simx_cmdheaderoffset_cmd))[0]) & simx_cmdmask;
	off = 0;
	while (off < cmdBufferSize) {
		cmd2Raw = extApi_endianConversionInt(
				((simxInt*) (cmdBuffer + off + simx_cmdheaderoffset_cmd))[0])
				& simx_cmdmask;
		if (cmd1Raw == cmd2Raw) { /* The commands are same. We need to check if the command data is same too */
			if ((cmd1Raw > simx_cmd4bytes_start)
					&& (cmd1Raw < simx_cmd8bytes_start)) {
				if (((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE))[0]
						== ((simxInt*) (cmdBuffer + off + SIMX_SUBHEADER_SIZE))[0])
					return (cmdBuffer + off);
			}
			if ((cmd1Raw > simx_cmd8bytes_start)
					&& (cmd1Raw < simx_cmd1string_start)) {
				if (((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE))[0]
						== ((simxInt*) (cmdBuffer + off + SIMX_SUBHEADER_SIZE))[0]) {
					if (((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE + 4))[0]
							== ((simxInt*) (cmdBuffer + off
									+ SIMX_SUBHEADER_SIZE + 4))[0])
						return (cmdBuffer + off);
				}
			}
			if ((cmd1Raw > simx_cmd1string_start)
					&& (cmd1Raw < simx_cmd4bytes2strings_start)) {
				if (extApi_areStringsSame(
						(simxChar*) cmdPtr + SIMX_SUBHEADER_SIZE,
						(simxChar*) cmdBuffer + off + SIMX_SUBHEADER_SIZE) != 0)
					return (cmdBuffer + off);
			}
			if ((cmd1Raw > simx_cmd4bytes2strings_start)
					&& (cmd1Raw < simx_cmd4bytes2strings_end)) {
				if (((simxInt*) (cmdPtr + SIMX_SUBHEADER_SIZE))[0]
						== ((simxInt*) (cmdBuffer + off + SIMX_SUBHEADER_SIZE))[0]) {
					if (extApi_areStringsSame(
							(simxChar*) cmdPtr + SIMX_SUBHEADER_SIZE + 4,
							(simxChar*) cmdBuffer + off + SIMX_SUBHEADER_SIZE
									+ 4) != 0) {
						tmp = extApi_getStringLength(
								(simxChar*) cmdPtr + SIMX_SUBHEADER_SIZE + 4)
								+ 1;
						if (extApi_areStringsSame(
								(simxChar*) cmdPtr + SIMX_SUBHEADER_SIZE + 4
										+ tmp,
								(simxChar*) cmdBuffer + off
										+ SIMX_SUBHEADER_SIZE + 4 + tmp) != 0)
							return (cmdBuffer + off);
					}
				}
			}
...
simxInt _getCmdDataSize(simxUChar *commandPointer) {
	simxInt retVal = 0;
	simxInt tmp;
	simxInt cmdRaw = extApi_endianConversionInt(
			((simxInt*) (commandPointer + simx_cmdheaderoffset_cmd))[0])
			& simx_cmdmask;
	if ((cmdRaw > simx_cmd4bytes_start) && (cmdRaw < simx_cmd8bytes_start))
		retVal = 4;
	if ((cmdRaw > simx_cmd8bytes_start) && (cmdRaw < simx_cmd1string_start))
		retVal = 8;
	if ((cmdRaw > simx_cmd1string_start)
			&& (cmdRaw < simx_cmd4bytes2strings_start))
		retVal = extApi_getStringLength(
				(simxChar*) commandPointer + SIMX_SUBHEADER_SIZE) + 1;
	if ((cmdRaw > simx_cmd4bytes2strings_start)
			&& (cmdRaw < simx_cmd4bytes2strings_end)) {
		tmp = extApi_getStringLength(
				(simxChar*) commandPointer + SIMX_SUBHEADER_SIZE + 4) + 1;
		retVal = 4 + tmp
				+ extApi_getStringLength(
						(simxChar*) commandPointer + SIMX_SUBHEADER_SIZE + 4
								+ tmp) + 1;
	}
	return (retVal);
}

simxUChar _sendMessage_socketOrSharedMem(simxInt clientID,
		const simxUChar *message, simxInt messageSize, simxUChar usingSharedMem) { /* return 1: success */
	...
	if (usingSharedMem) { /* send the message via shared memory */
		if (extApi_send_sharedMem(clientID, message, messageSize)
				!= messageSize)
			return (0);
	} else { /* send the message via sockets */
		...
		while (s != 0) {
			...
			s -= sizeToSend;
			if (_sendSimplePacket_socket(clientID, message + ptr,
					(simxShort) sizeToSend, packetCount) == 0)
				return (0);
			ptr += sizeToSend;
		}
	}
...
simxUChar* _receiveReplyMessage_socketOrSharedMem(simxInt clientID,
		simxInt *messageSize, simxUChar usingSharedMem) { /* return 0: failure */
	simxInt i, result;
	simxInt cnt = 0;
	simxUChar *retBuff = 0;
	...
	if (usingSharedMem) { /* receive data via shared memory */
		return (extApi_recv_sharedMem(clientID, messageSize));
	} else { /* receive data via sockets */
		while (1) {
			...
			if (cnt == 0)
				retBuff = extApi_allocateBuffer(
						(1 + result) * (inDatSize + SOCKET_HEADER_LENGTH));
			for (i = 0; i < inDatSize; i++)
				retBuff[retBuffSize + i] = inDat[i];
			extApi_releaseBuffer(inDat);
			retBuffSize += inDatSize;
			if (result == 0) { /* ok, no more packets to receive */
...
simxUChar _sendSimplePacket_socket(simxInt clientID, const simxUChar *packet,
		simxShort packetLength, simxShort packetsLeft) {
	simxInt i;
	...
	/* Send the packet */
	if (extApi_send_socket(clientID, toSend,
			packetLength + SOCKET_HEADER_LENGTH)==packetLength+SOCKET_HEADER_LENGTH)
		return (1);
	return (0);
}

simxInt _receiveSimplePacket_socket(simxInt clientID, simxUChar **packet,
		simxShort *packetSize) { /* Returns the number of packets left to read if >=0, otherwise error */
	...
	while (totalReceived != SOCKET_HEADER_LENGTH) {
		nb = extApi_recv_socket(clientID, headerAndSize + totalReceived,
				SOCKET_HEADER_LENGTH - totalReceived);
		if (nb < 1)
			break;
		totalReceived += nb;
	...
	while (totalReceived != dataLength) {
		nb = extApi_recv_socket(clientID, packet[0] + totalReceived,
				dataLength - totalReceived);
		if (nb < 1)
			break;
		totalReceived += nb;
...
SIMX_THREAD_RET_TYPE _communicationThread(simxVoid *p) {
	...
	while (_communicationThreadRunning[clientID] != 0) { /* only the main thread can have this thread end! */

		/* printf("Trying to connect...\n"); */
		if (usingSharedMem) { /* using shared memory */
			connectionResult = extApi_connectToServer_sharedMem(clientID,
					_tempConnectionPort[clientID]);
		} else { /* using sockets */
			connectionResult = extApi_connectToServer_socket(clientID,
					_tempConnectionAddress[clientID],
					_tempConnectionPort[clientID]);
		}
		if (connectionResult == 1) {
			...
			while (_communicationThreadRunning[clientID] != 0) {
				...
				extApi_lockResources(clientID);
				waitBeforeSendingAgainWhenMessageIDArrived_copy =
						_waitBeforeSendingAgainWhenMessageIDArrived[clientID];
				extApi_unlockResources(clientID);
				...
				while (extApi_getTimeDiffInMs(lastTime)
						< _minCommunicationDelay[clientID])
					extApi_switchThread();
				lastTime = extApi_getTimeInMs();
				...
				tempBuffer = extApi_allocateBuffer(
						_messageToSend_dataSize[clientID]);
				...
				while (off < _splitCommandsToSend_dataSize[clientID]) {
					...
					pureDataSize = memSize - SIMX_SUBHEADER_SIZE
							- pureDataOffset0 - pureDataOffset1;
					if (pureDataSize > maxPureDataSize)
						pureDataSize = maxPureDataSize;
					tempBuffer = _appendChunkToBuffer(
							_splitCommandsToSend[clientID] + off,
							SIMX_SUBHEADER_SIZE + pureDataOffset0, tempBuffer,
							&tempBufferBufferSize, &tempBufferDataSize);
					tempBuffer = _appendChunkToBuffer(
							_splitCommandsToSend[clientID] + off
									+ SIMX_SUBHEADER_SIZE + pureDataOffset0
									+ pureDataOffset1, pureDataSize, tempBuffer,
							&tempBufferBufferSize, &tempBufferDataSize);
					((simxInt*) (tempBuffer + tempBufferDataSize - pureDataSize
							- pureDataOffset0 - SIMX_SUBHEADER_SIZE
							+ simx_cmdheaderoffset_mem_size))[0] =
							extApi_endianConversionInt(
									SIMX_SUBHEADER_SIZE + pureDataOffset0
											+ pureDataSize);
					if (SIMX_SUBHEADER_SIZE + pureDataOffset0 + pureDataOffset1
							+ pureDataSize >= memSize) { /* command completely sent, we can remove it */
						_removeChunkFromBuffer(_splitCommandsToSend[clientID],
								_splitCommandsToSend[clientID] + off, memSize,
								&_splitCommandsToSend_dataSize[clientID]);
					} else { /* command not yet completely sent. Keep it, but adjust the pure data offset1 */
						pureDataOffset1 += pureDataSize;
						((simxInt*) (_splitCommandsToSend[clientID] + off
								+ simx_cmdheaderoffset_pdata_offset1))[0] =
								extApi_endianConversionInt(pureDataOffset1);
						off += memSize;
					}
				}
				/* Set some message header values */
				tempBuffer[simx_headeroffset_version] = SIMX_VERSION;
				((simxInt*) (tempBuffer + simx_headeroffset_message_id))[0] =
						extApi_endianConversionInt(
								_nextMessageIDToSend[clientID]++);
				((simxInt*) (tempBuffer + simx_headeroffset_client_time))[0] =
						extApi_endianConversionInt(extApi_getTimeInMs());
				...
				((simxUShort*) (tempBuffer + simx_headeroffset_crc))[0] =
						extApi_endianConversionUShort(0);
				/* Send the message */
				if (_sendMessage_socketOrSharedMem(clientID, tempBuffer,
						tempBufferDataSize, usingSharedMem) != 1) {
				...
				replyData = _receiveReplyMessage_socketOrSharedMem(clientID,
						&replyDataSize, usingSharedMem);
				if (replyData == 0)
					break;

				/* Check the CRC */
				/* CRC calculation represents a bottleneck for large transmissions, and is anyway not needed with tcp or shared memory transmissions */
				crc = extApi_endianConversionUShort(
						((simxUShort*) (replyData + simx_headeroffset_crc))[0]);
				/* if (_getCRC(replyData+2,replyDataSize-2)==crc) */
				if (1) {
					/* Place the reply into the input buffer */
					tmp = extApi_endianConversionInt(
							((simxInt*) (replyData
									+ simx_headeroffset_message_id))[0]);

					if (replyDataSize > SIMX_HEADER_SIZE) { /* We received a non-empty message */
						extApi_lockResources(clientID);
						/* a) Create a new buffer that will hold the merged input data */
						tempBuffer = extApi_allocateBuffer(
								_messageReceived_bufferSize[clientID]);
						...
						while (off < replyDataSize) {
							memSize =
									extApi_endianConversionInt(
											((simxInt*) (replyData + off
													+ simx_cmdheaderoffset_mem_size))[0]);
							fullMemSize =
									extApi_endianConversionInt(
											((simxInt*) (replyData + off
													+ simx_cmdheaderoffset_full_mem_size))[0]);
							if (memSize == fullMemSize) { /* the full data was sent at once! */
								cmd =
										extApi_endianConversionInt(
												((simxInt*) (replyData + off
														+ simx_cmdheaderoffset_cmd))[0]);
								if ((cmd - (cmd & simx_cmdmask))
										!= simx_opmode_discontinue) /* only discontinue mode commands are not added */
										{
									tempBuffer =
											_appendCommandToBufferAndTakeIntoAccountPreviouslyReceivedData(
													replyData + off,
								...
								cmdPointer = _getSameCommandPointer(
										replyData + off,
										_messageReceived[clientID]
												+ SIMX_HEADER_SIZE,
										_messageReceived_dataSize[clientID]
												- SIMX_HEADER_SIZE);
								if (cmdPointer != 0) { /* unmark this command (we already added its newer version) */
									((simxInt*) (cmdPointer
											+ simx_cmdheaderoffset_cmd))[0] = 0;
								}
							} else { /* only partial data was sent */

								/* Try to merge the partial data with same data already present in the partial commands buffer */
								cmdPointer =
										_getSameCommandPointer(replyData + off,
												_splitCommandsReceived[clientID],
												_splitCommandsReceived_dataSize[clientID]);
								if (cmdPointer != 0) { /* there is previous partial data. Is it valid? */
									memSize2 =
											extApi_endianConversionInt(
													((simxInt*) (cmdPointer
															+ simx_cmdheaderoffset_mem_size))[0]);
									if (memSize2 != fullMemSize) { /* we cannot use the previous version, since it has a different size. Remove it */
										_removeChunkFromBuffer(
												_splitCommandsReceived[clientID],
												cmdPointer, memSize2,
												&_splitCommandsReceived_dataSize[clientID]);
										cmdPointer = 0;
								...
								if (cmdPointer == 0) { /* there is not yet similar data present. Just add empty space */
									_splitCommandsReceived[clientID] =
											_appendChunkToBuffer(0, fullMemSize,
													_splitCommandsReceived[clientID],
								...
								((simxInt*) (cmdPointer
										+ simx_cmdheaderoffset_mem_size))[0] =
										extApi_endianConversionInt(fullMemSize); /* Important!! */

								pureDataOffset0 =
										extApi_endianConversionUShort(
												((simxUShort*) (cmdPointer
														+ simx_cmdheaderoffset_pdata_offset0))[0]);
								for (i = 0; i < pureDataOffset0; i++)
									cmdPointer[SIMX_SUBHEADER_SIZE + i] =
											replyData[off + SIMX_SUBHEADER_SIZE
													+ i];
								...
								for (i = 0; i < pureDataSize; i++)
									cmdPointer[SIMX_SUBHEADER_SIZE
											+ pureDataOffset0 + pureDataOffset1
											+ i] = replyData[off
											+ SIMX_SUBHEADER_SIZE
											+ pureDataOffset0 + i];
								...
								if (SIMX_SUBHEADER_SIZE + pureDataOffset0
										+ pureDataOffset1 + pureDataSize
										>= fullMemSize) { /* yes!! Copy the data from the partial command buffer to the tempBuffer, and erase it from the partial command buffer */
							...
							cmd =
									extApi_endianConversionInt(
											((simxInt*) (_messageReceived[clientID]
													+ off
													+ simx_cmdheaderoffset_cmd))[0]);
							if (cmd != 0) { /* ok, this command was not unmarked. We add it */
								tempBuffer =
										_appendChunkToBuffer(
												_messageReceived[clientID]
														+ off,
												extApi_endianConversionInt(
														((simxInt*) (_messageReceived[clientID]
			...
			extApi_sleepMs(100);
		if (_tempDoNotReconnectOnceDisconnected[clientID]) { /* sit here until the other thread sets _communicationThreadRunning[clientID] to 0 */
			while (_communicationThreadRunning[clientID] != 0)
				extApi_sleepMs(100);
			break;
		}
...
simxInt _removeCommandReply_int(simxInt clientID, simxInt cmdRaw,
		simxInt intValue) {
	simxUChar *cmdPtr;
	simxInt retVal = simx_return_ok;
	extApi_lockResources(clientID);
...
simxInt _removeCommandReply_intint(simxInt clientID, simxInt cmdRaw,
		simxInt intValue1, simxInt intValue2) {
	simxUChar *cmdPtr;
	simxInt retVal = simx_return_ok;
	extApi_lockResources(clientID);
	cmdPtr = _getCommandPointer_ii(cmdRaw, intValue1, intValue2,
...
simxInt _removeCommandReply_string(simxInt clientID, simxInt cmdRaw,
...
simxInt _removeCommandReply_intstringstring(simxInt clientID, simxInt cmdRaw,
		simxInt intValue, const simxUChar *stringValue1,
...
EXTAPI_DLLEXPORT simxInt simxGetPingTime(simxInt clientID, simxInt *pingTime) {
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (0);
	res = simxGetIntegerParameter(clientID, sim_intparam_program_version,
			&dummyVal, simx_opmode_blocking); /* just a dummy command */
...
EXTAPI_DLLEXPORT simxInt simxSynchronousTrigger(simxInt clientID) {
	...
	_exec_null(clientID, simx_cmd_synchronous_next, simx_opmode_blocking, 0,
			&returnValue);
	return (returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxPauseCommunication(simxInt clientID,
		simxUChar pause) {
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (pause)
...
EXTAPI_DLLEXPORT simxInt simxGetInMessageInfo(simxInt clientID,
		simxInt infoType, simxInt *info) {
	simxInt retVal = -1;
	if (_communicationThreadRunning[clientID] == 0)
		return (-1);
	extApi_lockResources(clientID);
	if (_messageReceived_dataSize[clientID] >= SIMX_HEADER_SIZE) {
		if ((infoType == simx_headeroffset_message_id)
				|| (infoType == simx_headeroffset_client_time)
				|| (infoType == simx_headeroffset_server_time)) {
			info[0] = extApi_endianConversionInt(
					((simxInt*) (_messageReceived[clientID] + infoType))[0]);
			retVal = 1;
		}
		if (infoType == simx_headeroffset_scene_id) {
			info[0] = extApi_endianConversionUShort(
					((simxUShort*) (_messageReceived[clientID] + infoType))[0]);
			retVal = 1;
		}
		if ((infoType == simx_headeroffset_version)
				|| (infoType == simx_headeroffset_server_state)) {
			info[0] = (simxInt) ((simxUChar*) (_messageReceived[clientID]
					+ infoType))[0];
			retVal = 1;
		}
	}
...
EXTAPI_DLLEXPORT simxInt simxGetOutMessageInfo(simxInt clientID,
		simxInt infoType, simxInt *info) {
	simxInt retVal = -1;
...
EXTAPI_DLLEXPORT simxInt simxGetJointPosition(simxInt clientID,
		simxInt jointHandle, simxFloat *position, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_joint_position,
				jointHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_joint_position,
			operationMode, 0, jointHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		position[0] = _readPureDataFloat(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetJointPosition(simxInt clientID,
		simxInt jointHandle, simxFloat position, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_joint_position,
				jointHandle));
	_exec_int_float(clientID, simx_cmd_set_joint_position, operationMode, 0,
			jointHandle, position, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetJointMatrix(simxInt clientID,
		simxInt jointHandle, simxFloat *matrix, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, i;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_joint_matrix,
				jointHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_joint_matrix, operationMode,
			0, jointHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		for (i = 0; i < 12; i++)
...
EXTAPI_DLLEXPORT simxInt simxSetSphericalJointMatrix(simxInt clientID,
		simxInt jointHandle, simxFloat *matrix, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_spherical_joint_matrix, jointHandle));
	_exec_int_buffer(clientID, simx_cmd_set_spherical_joint_matrix,
			operationMode, 0, jointHandle, (simxUChar*) matrix, 4 * 12,
			&returnValue);
	return (returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxSetJointTargetVelocity(simxInt clientID,
		simxInt jointHandle, simxFloat targetVelocity, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_joint_target_velocity, jointHandle));
	_exec_int_float(clientID, simx_cmd_set_joint_target_velocity, operationMode,
			0, jointHandle, targetVelocity, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetJointTargetPosition(simxInt clientID,
		simxInt jointHandle, simxFloat targetPosition, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_joint_target_position, jointHandle));
	_exec_int_float(clientID, simx_cmd_set_joint_target_position, operationMode,
			0, jointHandle, targetPosition, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxReadProximitySensor(simxInt clientID,
		simxInt sensorHandle, simxUChar *detectionState,
		simxFloat *detectedPoint, simxInt *detectedObjectHandle,
		simxFloat *detectedSurfaceNormalVector, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_read_proximity_sensor, sensorHandle));
	dataPointer = _exec_int(clientID, simx_cmd_read_proximity_sensor,
			operationMode, 0, sensorHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		if (detectionState != 0)
		...
		if (detectedSurfaceNormalVector != 0) {
			detectedSurfaceNormalVector[0] = _readPureDataFloat(dataPointer, 0,
					17);
			detectedSurfaceNormalVector[1] = _readPureDataFloat(dataPointer, 0,
					21);
			detectedSurfaceNormalVector[2] = _readPureDataFloat(dataPointer, 0,
					25);
		}
	}
	return (returnValue);
...
EXTAPI_DLLEXPORT simxInt simxStartSimulation(simxInt clientID,
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_start_pause_stop_simulation, 0));
	_exec_int(clientID, simx_cmd_start_pause_stop_simulation, operationMode, 0,
			0, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxPauseSimulation(simxInt clientID,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_start_pause_stop_simulation, 1));
	_exec_int(clientID, simx_cmd_start_pause_stop_simulation, operationMode, 0,
			1, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxStopSimulation(simxInt clientID,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_start_pause_stop_simulation, 2));
	_exec_int(clientID, simx_cmd_start_pause_stop_simulation, operationMode, 0,
			2, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetObjectHandle(simxInt clientID,
		const simxChar *objectName, simxInt *handle, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_get_object_handle,
				(simxUChar*) objectName));
	dataPointer = _exec_string(clientID, simx_cmd_get_object_handle,
			operationMode, 0, (simxUChar*) objectName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		handle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetUIHandle(simxInt clientID,
		const simxChar *uiName, simxInt *handle, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_get_ui_handle,
				(simxUChar*) uiName));
	dataPointer = _exec_string(clientID, simx_cmd_get_ui_handle, operationMode,
			0, (simxUChar*) uiName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		handle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetVisionSensorImage(simxInt clientID,
		simxInt sensorHandle, simxInt *resolution, simxUChar **image,
	...
	dataPointer = _exec_int(clientID, cmd, operationMode, 0, sensorHandle,
			&returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		resolution[0] = _readPureDataInt(dataPointer, 0, 0);
		resolution[1] = _readPureDataInt(dataPointer, 0, 4);
		image[0] = dataPointer + SIMX_SUBHEADER_SIZE + 8
				+ _getCmdDataSize(dataPointer);
	}
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetVisionSensorImage(simxInt clientID,
		simxInt sensorHandle, simxUChar *image, simxInt bufferSize,
		simxUChar options, simxInt operationMode) {
	simxInt returnValue;
	simxInt cmd;
	if (options & 1)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, cmd, sensorHandle));
	_exec_int_buffer(clientID, cmd, operationMode, 0, sensorHandle, image,
			bufferSize, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetVisionSensorDepthBuffer(simxInt clientID,
		simxInt sensorHandle, simxInt *resolution, simxFloat **buffer,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
#ifdef ENDIAN_TEST
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_get_vision_sensor_depth_buffer, sensorHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_vision_sensor_depth_buffer,
			operationMode, 0, sensorHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		resolution[0] = _readPureDataInt(dataPointer, 0, 0);
		resolution[1] = _readPureDataInt(dataPointer, 0, 4);
		buffer[0] = (simxFloat*) (dataPointer + SIMX_SUBHEADER_SIZE + 8
				+ _getCmdDataSize(dataPointer));
#ifdef ENDIAN_TEST
...
EXTAPI_DLLEXPORT simxInt simxJointGetForce(simxInt clientID,
		simxInt jointHandle, simxFloat *force, simxInt operationMode) { /* DEPRECATED since V3.1.2 */
	return (simxGetJointForce(clientID, jointHandle, force, operationMode));
}

EXTAPI_DLLEXPORT simxInt simxGetJointForce(simxInt clientID,
		simxInt jointHandle, simxFloat *force, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_joint_force,
				jointHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_joint_force, operationMode,
			0, jointHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxSetJointForce(simxInt clientID,
		simxInt jointHandle, simxFloat force, simxInt operationMode) { // DEPRECATED
	return (simxSetJointMaxForce(clientID, jointHandle, force, operationMode));
}

EXTAPI_DLLEXPORT simxInt simxSetJointMaxForce(simxInt clientID,
		simxInt jointHandle, simxFloat force, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_joint_force,
				jointHandle));
	_exec_int_float(clientID, simx_cmd_set_joint_force, operationMode, 0,
			jointHandle, force, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetJointMaxForce(simxInt clientID,
		simxInt jointHandle, simxFloat *force, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_joint_max_force,
				jointHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_joint_max_force,
			operationMode, 0, jointHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		force[0] = _readPureDataFloat(dataPointer, 0, 0);
	return (returnValue);
...
EXTAPI_DLLEXPORT simxInt simxReadForceSensor(simxInt clientID,
		simxInt forceSensorHandle, simxUChar *state, simxFloat *forceVector,
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_read_force_sensor,
				forceSensorHandle));
	dataPointer = _exec_int(clientID, simx_cmd_read_force_sensor, operationMode,
			0, forceSensorHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		if (state != 0)
		...
		if (torqueVector != 0) {
			for (i = 0; i < 3; i++)
				torqueVector[i] = _readPureDataFloat(dataPointer, 0,
						1 + 12 + 4 * i);
		}
	}
	return (returnValue);
...
EXTAPI_DLLEXPORT simxInt simxBreakForceSensor(simxInt clientID,
		simxInt forceSensorHandle, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_break_force_sensor,
				forceSensorHandle));
	_exec_int(clientID, simx_cmd_break_force_sensor, operationMode, 0,
			forceSensorHandle, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxReadVisionSensor(simxInt clientID,
		simxInt sensorHandle, simxUChar *detectionState, simxFloat **auxValues,
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_read_vision_sensor,
				sensorHandle));
	dataPointer = _exec_int(clientID, simx_cmd_read_vision_sensor,
			operationMode, 0, sensorHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		if (detectionState != 0)
			detectionState[0] = _readPureDataChar(dataPointer, 0, 0);
...
EXTAPI_DLLEXPORT simxInt simxGetObjectParent(simxInt clientID,
		simxInt childObjectHandle, simxInt *parentObjectHandle,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_object_parent,
				childObjectHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_object_parent, operationMode,
			0, childObjectHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		parentObjectHandle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetObjectChild(simxInt clientID,
		simxInt parentObjectHandle, simxInt childIndex,
		simxInt *childObjectHandle, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID, simx_cmd_get_object_child,
				parentObjectHandle, childIndex));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_child,
			operationMode, 0, parentObjectHandle, childIndex, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		childObjectHandle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxTransferFile(simxInt clientID,
		const simxChar *filePathAndName, const simxChar *fileName_serverSide,
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_transfer_file,
				(simxUChar*) filePathAndName));
	...
	_exec_string_buffer(clientID, simx_cmd_transfer_file, operationMode, 0,
			(simxUChar*) fileName_serverSide, buffer, bufferLength,
			&returnValue);
...
EXTAPI_DLLEXPORT simxInt simxEraseFile(simxInt clientID,
		const simxChar *fileName_serverSide, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_erase_file,
				(simxUChar*) fileName_serverSide));
	_exec_string(clientID, simx_cmd_erase_file, operationMode, 0,
			(simxUChar*) fileName_serverSide, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxLoadModel(simxInt clientID,
		const simxChar *modelPathAndName, simxUChar options,
		simxInt *baseHandle, simxInt operationMode) {
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_load_model,
				(simxUChar*) modelPathAndName));
	if (options & 1) { /* give some pseudo-random name to the temp file: */
		...
		returnValue = simxTransferFile(clientID, modelPathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_blocking);
		if (returnValue == 0) {
			dataPointer = _exec_string(clientID, simx_cmd_load_model,
					operationMode, 0, (simxUChar*) tmpFileName, &returnValue);
			simxEraseFile(clientID, tmpFileName, simx_opmode_oneshot);
		}
		simxTransferFile(clientID, modelPathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_remove);
	} else
		dataPointer = _exec_string(clientID, simx_cmd_load_model, operationMode,
				0, (simxUChar*) modelPathAndName, &returnValue);
...
EXTAPI_DLLEXPORT simxInt simxLoadUI(simxInt clientID,
		const simxChar *uiPathAndName, simxUChar options, simxInt *count,
		simxInt **uiHandles, simxInt operationMode) {
	simxUChar *dataPointer = 0;
	simxInt returnValue, i;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_load_ui,
				(simxUChar*) uiPathAndName));
	if (options & 1) { /* give some pseudo-random name to the temp file: */
		tmpFileName[20] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[21] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[22] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[23] = '0' + (char) (extApi_rand() * 9.1f);
		returnValue = simxTransferFile(clientID, uiPathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_blocking);
		if (returnValue == 0) {
			dataPointer = _exec_string(clientID, simx_cmd_load_ui,
					operationMode, 0, (simxUChar*) tmpFileName, &returnValue);
			simxEraseFile(clientID, tmpFileName, simx_opmode_oneshot);
		}
		simxTransferFile(clientID, uiPathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_remove);
	} else
		dataPointer = _exec_string(clientID, simx_cmd_load_ui, operationMode, 0,
				(simxUChar*) uiPathAndName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		count[0] = _readPureDataInt(dataPointer, 0, 0);
		uiHandles[0] = (simxInt*) extApi_allocateBuffer(4 * count[0]);
		for (i = 0; i < count[0]; i++)
			((simxInt*) (uiHandles[0]))[i] = _readPureDataInt(dataPointer, 0,
					4 + 4 * i);
	}
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxLoadScene(simxInt clientID,
		const simxChar *scenePathAndName, simxUChar options,
		simxInt operationMode) {
	simxInt returnValue;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_load_scene,
				(simxUChar*) scenePathAndName));
	if (options & 1) { /* give some pseudo-random name to the temp file: */
		tmpFileName[20] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[21] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[22] = '0' + (char) (extApi_rand() * 9.1f);
		tmpFileName[23] = '0' + (char) (extApi_rand() * 9.1f);
		returnValue = simxTransferFile(clientID, scenePathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_blocking);
		if (returnValue == 0) {
			_exec_string(clientID, simx_cmd_load_scene, operationMode, 0,
					(simxUChar*) tmpFileName, &returnValue);
			simxEraseFile(clientID, tmpFileName, simx_opmode_oneshot);
		}
		simxTransferFile(clientID, scenePathAndName, tmpFileName,
				_replyWaitTimeoutInMs[clientID], simx_opmode_remove);
	} else
		_exec_string(clientID, simx_cmd_load_scene, operationMode, 0,
				(simxUChar*) scenePathAndName, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetUISlider(simxInt clientID, simxInt uiHandle,
		simxInt uiButtonID, simxInt *position, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID, simx_cmd_get_ui_slider,
				uiHandle, uiButtonID));
	dataPointer = _exec_intint(clientID, simx_cmd_get_ui_slider, operationMode,
			0, uiHandle, uiButtonID, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		position[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetUISlider(simxInt clientID, simxInt uiHandle,
		simxInt uiButtonID, simxInt position, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID, simx_cmd_set_ui_slider,
				uiHandle, uiButtonID));
	_exec_intint_int(clientID, simx_cmd_set_ui_slider, operationMode, 0,
			uiHandle, uiButtonID, position, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetUIEventButton(simxInt clientID,
		simxInt uiHandle, simxInt *uiEventButtonID, simxInt *auxValues,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_ui_event_button,
				uiHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_ui_event_button,
			operationMode, 0, uiHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxGetUIButtonProperty(simxInt clientID,
		simxInt uiHandle, simxInt uiButtonID, simxInt *prop,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_ui_button_property, uiHandle, uiButtonID));
	dataPointer = _exec_intint(clientID, simx_cmd_get_ui_button_property,
			operationMode, 0, uiHandle, uiButtonID, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		prop[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetUIButtonProperty(simxInt clientID,
		simxInt uiHandle, simxInt uiButtonID, simxInt prop,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_set_ui_button_property, uiHandle, uiButtonID));
	_exec_intint_int(clientID, simx_cmd_set_ui_button_property, operationMode,
			0, uiHandle, uiButtonID, prop, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxAddStatusbarMessage(simxInt clientID,
		const simxChar *message, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_add_statusbar_message, (simxUChar*) message));
	_exec_string(clientID, simx_cmd_add_statusbar_message, operationMode, 1,
			(simxUChar*) message, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxCreateDummy(simxInt clientID, simxFloat size,
		const simxUChar *colors, simxInt *objectHandle, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, i;
	...
	dataPointer = _exec_null_buffer(clientID, simx_cmd_create_dummy,
			operationMode, 1, buffer, 4 + 1 + 12, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxCallScriptFunction(simxInt clientID,
		const simxChar *scriptDescription, simxInt options,
	...
	simxInt returnValue;
	simxUChar *buffer;
	simxInt i, off, totStrSize, bufferSize, outIntC, outFloatC, outStringC,
			outBufferS;
	simxChar *outStr;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intstringstring(clientID,
				simx_cmd_aux_console_open, options,
	...
	bufferSize = 4 + 4 + 4 + 4 + 4 * (inIntCnt + inFloatCnt) + totStrSize
			+ inBufferSize;
	...
	for (i = 0; i < inFloatCnt; i++)
		((simxFloat*) buffer)[off + i] = extApi_endianConversionFloat(
				inFloat[i]);
	off += inFloatCnt;
	off *= 4;
	...
	dataPointer = _exec_intstringstring_buffer(clientID,
			simx_cmd_call_script_function, operationMode, 1, options,
	...
	if ((dataPointer != 0) && (returnValue == 0)) {
		...
		if ((outIntCnt != 0) && (outInt != 0)) {
			outIntCnt[0] = outIntC;
			outInt[0] = ((simxInt*) (dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + off)); /* little/big endian conversion happened on the server side */
		}
		off += outIntC * 4;

		if ((outFloatCnt != 0) && (outFloat != 0)) {
			outFloatCnt[0] = outFloatC;
			outFloat[0] = ((simxFloat*) (dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + off)); /* little/big endian conversion happened on the server side */
		}
		off += outFloatC * 4;

		outStr = ((simxChar*) (dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer) + off));
		...
		if ((outBufferSize != 0) && (outBuffer != 0)) {
			...
			outBuffer[0] = ((simxUChar*) (dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + off));
		}
	}
	return (returnValue);
...
EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsoleOpen(simxInt clientID,
		const simxChar *title, simxInt maxLines, simxInt mode,
		simxInt *position, simxInt *size, simxFloat *textColor,
		simxFloat *backgroundColor, simxInt *consoleHandle,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_aux_console_open,
				(simxUChar*) title));
	...
	if (backgroundColor != 0) {
		((simxFloat*) buffer)[9] = extApi_endianConversionFloat(
				backgroundColor[0]);
		((simxFloat*) buffer)[10] = extApi_endianConversionFloat(
				backgroundColor[1]);
		((simxFloat*) buffer)[11] = extApi_endianConversionFloat(
				backgroundColor[2]);
	}
	dataPointer = _exec_string_buffer(clientID, simx_cmd_aux_console_open,
			operationMode, 0, (simxUChar*) title, buffer, 12 * 4, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		consoleHandle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsoleClose(simxInt clientID,
		simxInt consoleHandle, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_aux_console_close,
				consoleHandle));
	_exec_int(clientID, simx_cmd_aux_console_close, operationMode, 0,
			consoleHandle, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsolePrint(simxInt clientID,
		simxInt consoleHandle, const simxChar *txt, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_aux_console_print,
				consoleHandle));
	if (txt == 0)
		_exec_int_buffer(clientID, simx_cmd_aux_console_print, operationMode, 1,
				consoleHandle, 0, 0, &returnValue);
	else
		_exec_int_buffer(clientID, simx_cmd_aux_console_print, operationMode, 1,
				consoleHandle, (simxUChar*) txt,
				extApi_getStringLength(txt) + 1, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxAuxiliaryConsoleShow(simxInt clientID,
		simxInt consoleHandle, simxUChar showState, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_aux_console_show,
				consoleHandle));
	_exec_int_int(clientID, simx_cmd_aux_console_show, operationMode, 0,
			consoleHandle, (simxInt) showState, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetObjectOrientation(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
		simxFloat *eulerAngles, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_orientation2, objectHandle,
				relativeToObjectHandle));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_orientation2,
			operationMode, 0, objectHandle, relativeToObjectHandle,
			&returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxGetObjectPosition(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_position2, objectHandle,
				relativeToObjectHandle));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_position2,
			operationMode, 0, objectHandle, relativeToObjectHandle,
			&returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		position[0] = _readPureDataFloat(dataPointer, 0, 0);
...
EXTAPI_DLLEXPORT simxInt simxGetObjectQuaternion(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
		simxFloat *quaternion, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_quaternion, objectHandle,
				relativeToObjectHandle));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_quaternion,
			operationMode, 0, objectHandle, relativeToObjectHandle,
			&returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxSetObjectOrientation(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_object_orientation, objectHandle));
	...
	_exec_int_buffer(clientID, simx_cmd_set_object_orientation, operationMode,
			0, objectHandle, buffer, 4 * 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetObjectPosition(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
		const simxFloat *position, simxInt operationMode) {
	simxInt returnValue;
	simxUChar buffer[4 * 4];
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_object_position,
				objectHandle));
	((simxInt*) buffer)[0] = extApi_endianConversionInt(relativeToObjectHandle);
	...
	_exec_int_buffer(clientID, simx_cmd_set_object_position, operationMode, 0,
			objectHandle, buffer, 4 * 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetObjectQuaternion(simxInt clientID,
		simxInt objectHandle, simxInt relativeToObjectHandle,
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_object_quaternion, objectHandle));
	((simxInt*) buffer)[0] = extApi_endianConversionInt(relativeToObjectHandle);
	...
	_exec_int_buffer(clientID, simx_cmd_set_object_quaternion, operationMode, 0,
			objectHandle, buffer, 5 * 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetObjectParent(simxInt clientID,
		simxInt objectHandle, simxInt parentObject, simxUChar keepInPlace,
		simxInt operationMode) {
	simxInt returnValue;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_object_parent,
				objectHandle));
	((simxInt*) buffer)[0] = extApi_endianConversionInt(parentObject);
	buffer[4] = keepInPlace;
	_exec_int_buffer(clientID, simx_cmd_set_object_parent, operationMode, 0,
			objectHandle, buffer, 4 + 1, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetUIButtonLabel(simxInt clientID,
		simxInt uiHandle, simxInt uiButtonID, const simxChar *upStateLabel,
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_set_ui_button_label, uiHandle, uiButtonID));
	...
	_exec_intint_buffer(clientID, simx_cmd_set_ui_button_label, operationMode,
			0, uiHandle, uiButtonID, buffer, strL1 + strL2 + 1 + 1,
			&returnValue);
	extApi_releaseBuffer(buffer);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetLastErrors(simxInt clientID, simxInt *errorCnt,
		simxChar **errorStrings, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	...
	dataPointer = _exec_null(clientID, simx_cmd_get_last_errors, operationMode,
			0, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		errorCnt[0] = _readPureDataInt(dataPointer, 0, 0);
		errorStrings[0] = (simxChar*) dataPointer + SIMX_SUBHEADER_SIZE + 4
				+ _getCmdDataSize(dataPointer);
	}
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetArrayParameter(simxInt clientID,
		simxInt paramIdentifier, simxFloat *paramValues, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, i;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_array_parameter,
				paramIdentifier));
	dataPointer = _exec_int(clientID, simx_cmd_get_array_parameter,
			operationMode, 0, paramIdentifier, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		for (i = 0; i < 3; i++)
...
EXTAPI_DLLEXPORT simxInt simxSetArrayParameter(simxInt clientID,
		simxInt paramIdentifier, const simxFloat *paramValues,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_array_parameter,
				paramIdentifier));
	_exec_int_buffer(clientID, simx_cmd_set_array_parameter, operationMode, 0,
			paramIdentifier, (simxUChar*) paramValues, 3 * 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetBooleanParameter(simxInt clientID,
		simxInt paramIdentifier, simxUChar *paramValue, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_get_boolean_parameter, paramIdentifier));
	dataPointer = _exec_int(clientID, simx_cmd_get_boolean_parameter,
			operationMode, 0, paramIdentifier, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxSetBooleanParameter(simxInt clientID,
		simxInt paramIdentifier, simxUChar paramValue, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_boolean_parameter, paramIdentifier));
	_exec_int_int(clientID, simx_cmd_set_boolean_parameter, operationMode, 0,
			paramIdentifier, (simxInt) paramValue, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetIntegerParameter(simxInt clientID,
		simxInt paramIdentifier, simxInt *paramValue, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_get_integer_parameter, paramIdentifier));
	dataPointer = _exec_int(clientID, simx_cmd_get_integer_parameter,
			operationMode, 0, paramIdentifier, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxSetIntegerParameter(simxInt clientID,
		simxInt paramIdentifier, simxInt paramValue, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_integer_parameter, paramIdentifier));
	_exec_int_int(clientID, simx_cmd_set_integer_parameter, operationMode, 0,
			paramIdentifier, paramValue, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetFloatingParameter(simxInt clientID,
		simxInt paramIdentifier, simxFloat *paramValue, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_get_floating_parameter, paramIdentifier));
	dataPointer = _exec_int(clientID, simx_cmd_get_floating_parameter,
			operationMode, 0, paramIdentifier, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxSetFloatingParameter(simxInt clientID,
		simxInt paramIdentifier, simxFloat paramValue, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID,
				simx_cmd_set_floating_parameter, paramIdentifier));
	_exec_int_float(clientID, simx_cmd_set_floating_parameter, operationMode, 0,
			paramIdentifier, paramValue, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetStringParameter(simxInt clientID,
		simxInt paramIdentifier, simxChar **paramValue, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_string_parameter,
				paramIdentifier));
	dataPointer = _exec_int(clientID, simx_cmd_get_string_parameter,
			operationMode, 0, paramIdentifier, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		paramValue[0] = (simxChar*) dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetCollisionHandle(simxInt clientID,
		const simxChar *collisionObjectName, simxInt *handle,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_get_collision_handle, (simxUChar*) collisionObjectName));
	dataPointer = _exec_string(clientID, simx_cmd_get_collision_handle,
			operationMode, 0, (simxUChar*) collisionObjectName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		handle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetDistanceHandle(simxInt clientID,
		const simxChar *distanceObjectName, simxInt *handle,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_get_distance_handle, (simxUChar*) distanceObjectName));
	dataPointer = _exec_string(clientID, simx_cmd_get_distance_handle,
			operationMode, 0, (simxUChar*) distanceObjectName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		handle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetCollectionHandle(simxInt clientID,
		const simxChar *collectionName, simxInt *handle, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_get_collection_handle, (simxUChar*) collectionName));
	dataPointer = _exec_string(clientID, simx_cmd_get_collection_handle,
			operationMode, 0, (simxUChar*) collectionName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		handle[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxReadCollision(simxInt clientID,
		simxInt collisionObjectHandle, simxUChar *collisionState,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_read_collision,
				collisionObjectHandle));
	dataPointer = _exec_int(clientID, simx_cmd_read_collision, operationMode, 0,
			collisionObjectHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		collisionState[0] = (simxUChar) _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxReadDistance(simxInt clientID,
		simxInt distanceObjectHandle, simxFloat *minimumDistance,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_read_distance,
				distanceObjectHandle));
	dataPointer = _exec_int(clientID, simx_cmd_read_distance, operationMode, 0,
			distanceObjectHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		minimumDistance[0] = _readPureDataFloat(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxRemoveObject(simxInt clientID,
		simxInt objectHandle, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_remove_object,
				objectHandle));
	_exec_int(clientID, simx_cmd_remove_object, operationMode, 0, objectHandle,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxRemoveModel(simxInt clientID, simxInt objectHandle,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_remove_model,
				objectHandle));
	_exec_int(clientID, simx_cmd_remove_model, operationMode, 0, objectHandle,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxRemoveUI(simxInt clientID, simxInt uiHandle,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	...
	_exec_int(clientID, simx_cmd_remove_ui, operationMode, 0, uiHandle,
			&returnValue);
	return (returnValue);
}
...
EXTAPI_DLLEXPORT simxInt simxGetObjects(simxInt clientID, simxInt objectType,
		simxInt *objectCount, simxInt **objectHandles, simxInt operationMode) {
	simxUChar *dataPointer;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_objects,
				objectType));
	dataPointer = _exec_int(clientID, simx_cmd_get_objects, operationMode, 0,
...
EXTAPI_DLLEXPORT simxInt simxDisplayDialog(simxInt clientID,
		const simxChar *titleText, const simxChar *mainText, simxInt dialogType,
		const simxChar *initialText, const simxFloat *titleColors,
		const simxFloat *dialogColors, simxInt *dialogHandle, simxInt *uiHandle,
		simxInt operationMode) {
	simxUChar *dataPointer;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_display_dialog,
				(simxUChar*) titleText));
	...
	dataPointer = _exec_string_buffer(clientID, simx_cmd_display_dialog,
			operationMode, 0, (simxUChar*) titleText, buffer,
			str1L + 1 + 4 + str2L + 1 + 4 * 6 + 4 * 6, &returnValue);
	extApi_releaseBuffer(buffer);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxEndDialog(simxInt clientID, simxInt dialogHandle,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_end_dialog,
				dialogHandle));
	_exec_int(clientID, simx_cmd_end_dialog, operationMode, 0, dialogHandle,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetDialogInput(simxInt clientID,
		simxInt dialogHandle, simxChar **inputText, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_dialog_input,
				dialogHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_dialog_input, operationMode,
			0, dialogHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		inputText[0] = (simxChar*) dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer);
	}
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetDialogResult(simxInt clientID,
		simxInt dialogHandle, simxInt *result, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_dialog_result,
				dialogHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_dialog_result, operationMode,
			0, dialogHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxCopyPasteObjects(simxInt clientID,
		const simxInt *objectHandles, simxInt objectCount,
		simxInt **newObjectHandles, simxInt *newObjectCount,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, off;
#ifdef ENDIAN_TEST
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_copy_paste_objects,
				0));
	dataPointer = _exec_int_buffer(clientID, simx_cmd_copy_paste_objects,
			operationMode, 1, 0, (simxUChar*) objectHandles, objectCount * 4,
			&returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
...
EXTAPI_DLLEXPORT simxInt simxGetObjectSelection(simxInt clientID,
		simxInt **objectHandles, simxInt *objectCount, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, off;
#ifdef ENDIAN_TEST
    simxInt i;
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_null(clientID,
				simx_cmd_get_object_selection));
	dataPointer = _exec_null(clientID, simx_cmd_get_object_selection,
			operationMode, 0, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		objectCount[0] = _readPureDataInt(dataPointer, 0, 0);
		off = SIMX_SUBHEADER_SIZE + _getCmdDataSize(dataPointer) + 4;
#ifdef ENDIAN_TEST
...
EXTAPI_DLLEXPORT simxInt simxSetObjectSelection(simxInt clientID,
		const simxInt *objectHandles, simxInt objectCount,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_object_selection,
				0));
	_exec_int_buffer(clientID, simx_cmd_set_object_selection, operationMode, 0,
			0, (simxUChar*) objectHandles, objectCount * 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxClearFloatSignal(simxInt clientID,
		const simxChar *signalName, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_clear_float_signal, (simxUChar*) signalName));
	_exec_string(clientID, simx_cmd_clear_float_signal, operationMode, 0,
			(simxUChar*) signalName, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxClearIntegerSignal(simxInt clientID,
		const simxChar *signalName, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_clear_integer_signal, (simxUChar*) signalName));
	_exec_string(clientID, simx_cmd_clear_integer_signal, operationMode, 0,
			(simxUChar*) signalName, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxClearStringSignal(simxInt clientID,
		const simxChar *signalName, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_clear_string_signal, (simxUChar*) signalName));
	_exec_string(clientID, simx_cmd_clear_string_signal, operationMode, 0,
			(simxUChar*) signalName, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetFloatSignal(simxInt clientID,
		const simxChar *signalName, simxFloat *signalValue,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_get_float_signal,
				(simxUChar*) signalName));
	dataPointer = _exec_string(clientID, simx_cmd_get_float_signal,
			operationMode, 0, (simxUChar*) signalName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		signalValue[0] = _readPureDataFloat(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetIntegerSignal(simxInt clientID,
		const simxChar *signalName, simxInt *signalValue, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_get_integer_signal, (simxUChar*) signalName));
	dataPointer = _exec_string(clientID, simx_cmd_get_integer_signal,
			operationMode, 0, (simxUChar*) signalName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxGetStringSignal(simxInt clientID,
		const simxChar *signalName, simxUChar **signalValue,
	...
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_get_string_signal,
				(simxUChar*) signalName));
	dataPointer = _exec_string(clientID, simx_cmd_get_string_signal,
			operationMode, 0, (simxUChar*) signalName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		signalValue[0] = dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer);
		signalLength[0] =
				extApi_endianConversionInt(
						((simxInt*) (dataPointer
...
EXTAPI_DLLEXPORT simxInt simxGetAndClearStringSignal(simxInt clientID,
		const simxChar *signalName, simxUChar **signalValue,
		simxInt *signalLength, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_get_and_clear_string_signal, (simxUChar*) signalName));
	dataPointer = _exec_string(clientID, simx_cmd_get_and_clear_string_signal,
			operationMode, 0, (simxUChar*) signalName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		signalValue[0] = dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer);
		...
		if (operationMode == simx_opmode_buffer)/* &&(signalLength[0]>0)) */
			_removeCommandReply_string(clientID,
...
EXTAPI_DLLEXPORT simxInt simxReadStringStream(simxInt clientID,
		const simxChar *signalName, simxUChar **signalValue,
		simxInt *signalLength, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_read_string_stream, (simxUChar*) signalName));
	...
	dataPointer = _exec_string(clientID, simx_cmd_read_string_stream,
			operationMode, 0, (simxUChar*) signalName, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		signalValue[0] = dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer);
...
EXTAPI_DLLEXPORT simxInt simxSetFloatSignal(simxInt clientID,
		const simxChar *signalName, simxFloat signalValue,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_set_float_signal,
				(simxUChar*) signalName));
	signalValue = extApi_endianConversionFloat(signalValue);
	_exec_string_buffer(clientID, simx_cmd_set_float_signal, operationMode, 0,
			(simxUChar*) signalName, (simxUChar*) &signalValue, 4,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetIntegerSignal(simxInt clientID,
		const simxChar *signalName, simxInt signalValue, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_set_integer_signal, (simxUChar*) signalName));
	signalValue = extApi_endianConversionInt(signalValue);
	_exec_string_buffer(clientID, simx_cmd_set_integer_signal, operationMode, 0,
			(simxUChar*) signalName, (simxUChar*) &signalValue, 4,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetStringSignal(simxInt clientID,
		const simxChar *signalName, const simxUChar *signalValue,
		simxInt signalLength, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID, simx_cmd_set_string_signal,
				(simxUChar*) signalName));
	_exec_string_buffer(clientID, simx_cmd_set_string_signal, operationMode, 0,
			(simxUChar*) signalName, (simxUChar*) signalValue, signalLength,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxAppendStringSignal(simxInt clientID,
		const simxChar *signalName, const simxUChar *signalValue,
		simxInt signalLength, simxInt operationMode) { /* since 31.1.2013: append mode */
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_string(clientID,
				simx_cmd_append_string_signal, (simxUChar*) signalName));
	_exec_string_buffer(clientID, simx_cmd_append_string_signal, operationMode,
			1, (simxUChar*) signalName, (simxUChar*) signalValue, signalLength,
			&returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxWriteStringStream(simxInt clientID,
		const simxChar *signalName, const simxUChar *signalValue,
		simxInt signalLength, simxInt operationMode) { /* this is just a convenience function */
	return (simxAppendStringSignal(clientID, signalName, signalValue,
			signalLength, operationMode));
}

EXTAPI_DLLEXPORT simxInt simxGetObjectFloatParameter(simxInt clientID,
		simxInt objectHandle, simxInt parameterID, simxFloat *parameterValue,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_float_parameter, objectHandle, parameterID));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_float_parameter,
			operationMode, 0, objectHandle, parameterID, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		parameterValue[0] = _readPureDataFloat(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetObjectFloatParameter(simxInt clientID,
		simxInt objectHandle, simxInt parameterID, simxFloat parameterValue,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_set_object_float_parameter, objectHandle, parameterID));
	parameterValue = extApi_endianConversionFloat(parameterValue);
	_exec_intint_buffer(clientID, simx_cmd_set_object_float_parameter,
			operationMode, 0, objectHandle, parameterID,
			(simxUChar*) &parameterValue, 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetObjectIntParameter(simxInt clientID,
		simxInt objectHandle, simxInt parameterID, simxInt *parameterValue,
		simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_int_parameter, objectHandle, parameterID));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_int_parameter,
			operationMode, 0, objectHandle, parameterID, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
...
EXTAPI_DLLEXPORT simxInt simxSetObjectIntParameter(simxInt clientID,
		simxInt objectHandle, simxInt parameterID, simxInt parameterValue,
		simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
	...
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_set_object_int_parameter, objectHandle, parameterID));
	parameterValue = extApi_endianConversionInt(parameterValue);
	_exec_intint_buffer(clientID, simx_cmd_set_object_int_parameter,
			operationMode, 0, objectHandle, parameterID,
			(simxUChar*) &parameterValue, 4, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetModelProperty(simxInt clientID,
		simxInt objectHandle, simxInt *prop, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_model_property,
				objectHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_model_property,
			operationMode, 0, objectHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0))
		prop[0] = _readPureDataInt(dataPointer, 0, 0);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxSetModelProperty(simxInt clientID,
		simxInt objectHandle, simxInt prop, simxInt operationMode) {
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_set_model_property,
				objectHandle));
	_exec_int_int(clientID, simx_cmd_set_model_property, operationMode, 0,
			objectHandle, prop, &returnValue);
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxQuery(simxInt clientID, const simxChar *signalName,
		const simxUChar *signalValue, simxInt signalLength,
		const simxChar *retSignalName, simxUChar **retSignalValue,
		simxInt *retSignalLength, simxInt timeOutInMs) {
	simxInt err;
	simxInt startTime = extApi_getTimeInMs();
	...
	simxGetAndClearStringSignal(clientID, retSignalName, retSignalValue,
			retSignalLength, simx_opmode_streaming);
	err = simxSetStringSignal(clientID, signalName, signalValue, signalLength,
			simx_opmode_blocking);
	if (err != 0)
		return (err);
	while (extApi_getTimeDiffInMs(startTime) < timeOutInMs) {
		if (simxGetAndClearStringSignal(clientID, retSignalName, retSignalValue,
				retSignalLength, simx_opmode_buffer) == 0)
			return (0); /* success */
		extApi_switchThread();
	}
...
EXTAPI_DLLEXPORT simxInt simxGetObjectGroupData(simxInt clientID,
		simxInt objectType, simxInt dataType, simxInt *handlesCount,
		simxInt **handles, simxInt *intDataCount, simxInt **intData,
		simxInt *floatDataCount, simxFloat **floatData,
		simxInt *stringDataCount, simxChar **stringData, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue, additionalOffset, intDataCount_, floatDataCount_,
			stringDataCount_;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_intint(clientID,
				simx_cmd_get_object_group_data, objectType, dataType));
	dataPointer = _exec_intint(clientID, simx_cmd_get_object_group_data,
			operationMode, 0, objectType, dataType, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		...
		handles[0] = ((simxInt*) (dataPointer + SIMX_SUBHEADER_SIZE
				+ _getCmdDataSize(dataPointer) + additionalOffset)); /* little/big endian conversion happened on the server side */
		additionalOffset += handlesCount[0] * 4;

		if (intData != NULL)
			intData[0] = ((simxInt*) (dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + additionalOffset)); /* little/big endian conversion happened on the server side */
		additionalOffset += intDataCount_ * 4;

		if (floatData != NULL)
			floatData[0] = ((simxFloat*) (dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + additionalOffset)); /* little/big endian conversion happened on the server side */
		additionalOffset += floatDataCount_ * 4;

		if (stringData != NULL)
			stringData[0] = (simxChar*) dataPointer + SIMX_SUBHEADER_SIZE
					+ _getCmdDataSize(dataPointer) + additionalOffset;
	}
	return (returnValue);
}

EXTAPI_DLLEXPORT simxInt simxGetObjectVelocity(simxInt clientID,
		simxInt objectHandle, simxFloat *linearVelocity,
		simxFloat *angularVelocity, simxInt operationMode) {
	simxUChar *dataPointer;
	simxInt returnValue;
	if (_communicationThreadRunning[clientID] == 0)
		return (simx_return_initialize_error_flag);
	if (operationMode == simx_opmode_remove)
		return (_removeCommandReply_int(clientID, simx_cmd_get_object_velocity,
				objectHandle));
	dataPointer = _exec_int(clientID, simx_cmd_get_object_velocity,
			operationMode, 0, objectHandle, &returnValue);
	if ((dataPointer != 0) && (returnValue == 0)) {
		if (linearVelocity != 0) {
...
EXTAPI_DLLEXPORT simxInt mtlb_simxSetJointPosition(simxInt clientID,
		simxInt jointHandle, simxFloat *position, simxInt operationMode) {
	return (simxSetJointPosition(clientID, jointHandle, position[0],
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetJointTargetVelocity(simxInt clientID,
		simxInt jointHandle, simxFloat *targetVelocity, simxInt operationMode) {
	return (simxSetJointTargetVelocity(clientID, jointHandle, targetVelocity[0],
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetJointTargetPosition(simxInt clientID,
		simxInt jointHandle, simxFloat *targetPosition, simxInt operationMode) {
	return (simxSetJointTargetPosition(clientID, jointHandle, targetPosition[0],
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetJointForce(simxInt clientID,
		simxInt jointHandle, simxFloat *force, simxInt operationMode) {
	return (simxSetJointMaxForce(clientID, jointHandle, force[0], operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetJointMaxForce(simxInt clientID,
		simxInt jointHandle, simxFloat *force, simxInt operationMode) {
	return (simxSetJointMaxForce(clientID, jointHandle, force[0], operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetFloatSignal(simxInt clientID,
		const simxChar *signalName, simxFloat *signalValue,
		simxInt operationMode) {
	return (simxSetFloatSignal(clientID, signalName, signalValue[0],
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetObjectFloatParameter(simxInt clientID,
		simxInt objectHandle, simxInt parameterID, simxFloat *parameterValue,
		simxInt operationMode) {
	return (simxSetObjectFloatParameter(clientID, objectHandle, parameterID,
			parameterValue[0], operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxSetFloatingParameter(simxInt clientID,
		simxInt paramIdentifier, simxFloat *paramValue, simxInt operationMode) {
	return (simxSetFloatingParameter(clientID, paramIdentifier, paramValue[0],
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxCreateDummy(simxInt clientID, simxFloat *size,
		const simxUChar *colors, simxInt *objectHandle, simxInt operationMode) {
	return (simxCreateDummy(clientID, size[0], colors, objectHandle,
			operationMode));
}

EXTAPI_DLLEXPORT simxInt mtlb_simxReadProximitySensor(
		simxInt *clientIDandSensorHandle, simxUChar *detectionState,
		simxFloat *detectedPoint, simxInt *detectedObjectHandle,
		simxFloat *detectedSurfaceNormalVector, simxInt operationMode) {
	int ret;
	ret = simxReadProximitySensor(clientIDandSensorHandle[0],
			clientIDandSensorHandle[1], detectionState, detectedPoint,
			detectedObjectHandle, detectedSurfaceNormalVector, operationMode);
	return (ret);
}

EXTAPI_DLLEXPORT simxInt mtlb_simxAuxiliaryConsoleOpen(
		simxInt *clientIDandMaxLinesAndModeAndPositionAndSize,
		const simxChar *title, simxFloat *textColor, simxFloat *backgroundColor,
		simxInt *consoleHandle, simxInt operationMode) {
	int ret;
	int p_[2];
	int s_[2];
	...
	if (s_[0] < -9999)
		s = NULL;
	ret = simxAuxiliaryConsoleOpen(
			clientIDandMaxLinesAndModeAndPositionAndSize[0], title,
			clientIDandMaxLinesAndModeAndPositionAndSize[1],
			clientIDandMaxLinesAndModeAndPositionAndSize[2], p, s, textColor,
			backgroundColor, consoleHandle, operationMode);
	return (ret);
}

EXTAPI_DLLEXPORT simxInt mtlb_simxDisplayDialog(
		simxInt *clientIDandDlgTypeAndOperationMode, const simxChar *titleText,
		const simxChar *mainText, const simxChar *initialText,
	...
	if (dialogColors_[0] < -9999.0f)
		dialogColors = NULL;
	ret = simxDisplayDialog(clientIDandDlgTypeAndOperationMode[0], titleText,
			mainText, clientIDandDlgTypeAndOperationMode[1], initialText,
			titleColors, dialogColors, dialogHandleAndUiHandle,
			dialogHandleAndUiHandle + 1, clientIDandDlgTypeAndOperationMode[2]);
	return (ret);
}

EXTAPI_DLLEXPORT simxInt mtlb_simxQuery(
		simxInt *clientIDandSignalLengthAndTimeOutInMs,
		const simxChar *signalName, const simxUChar *signalValue,
		const simxChar *retSignalName, simxUChar **retSignalValue,
		simxInt *retSignalLength) {
	int ret;
	ret = simxQuery(clientIDandSignalLengthAndTimeOutInMs[0], signalName,
			signalValue, clientIDandSignalLengthAndTimeOutInMs[1],
			retSignalName, retSignalValue, retSignalLength,
			clientIDandSignalLengthAndTimeOutInMs[2]);
	return (ret);
}

EXTAPI_DLLEXPORT simxInt mtlb_simxGetObjectGroupData(
		simxInt *clientIDandObjectTypeAndDataTypeAndOperationMode,
		...
		simxChar **stringData) {
	int ret;
	ret = simxGetObjectGroupData(
			clientIDandObjectTypeAndDataTypeAndOperationMode[0],
			clientIDandObjectTypeAndDataTypeAndOperationMode[1],
			clientIDandObjectTypeAndDataTypeAndOperationMode[2],
			handlesCountAndIntDataCountAndFloatDataCountAndStringDataCount + 0,
			handles,
			handlesCountAndIntDataCountAndFloatDataCountAndStringDataCount + 1,
			intData,
			handlesCountAndIntDataCountAndFloatDataCountAndStringDataCount + 2,
			floatData,
			handlesCountAndIntDataCountAndFloatDataCountAndStringDataCount + 3,
			stringData, clientIDandObjectTypeAndDataTypeAndOperationMode[3]);
	return (ret);
}

EXTAPI_DLLEXPORT simxInt mtlb_simxCallScriptFunction_a(
		const simxInt *variousIntsIn,
		const simxChar *scriptDescriptionAndFunctionName, const simxInt *inInt,
		const simxFloat *inFloat, const simxChar *inString,
...
EXTAPI_DLLEXPORT simxInt mtlb_simxCallScriptFunction_b(simxInt clientID,
		simxInt *variousIntsOut, simxInt **outInt, simxFloat **outFloat,
	...
	ret = simxCallScriptFunction(clientID, scriptDescription, options, funcName,
			inIntCnt, inInts, inFloatCnt, inFloats, inStringCnt, inStrings,
			inBufferSize, inBuffer, &variousIntsOut[0], outInt,
			&variousIntsOut[1], outFloat, &variousIntsOut[2], outString,
			&variousIntsOut[3], outBuffer, opMode);
